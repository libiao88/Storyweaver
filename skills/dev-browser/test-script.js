...
const page = await client.page("storyweaver-test", { viewport: { width: 1920, height: 1080 } });
...
// 检查是否显示标题
const titleElement = await page.waitForSelector("h1, h2, h3");
...
// 截图
await page.screenshot({ path: "tmp/TC-001首页加载.png" });
...
try {
  ...
  // 截图记录Tab导航
  await page.screenshot({ path: "tmp/TC-003Tab导航.png" });
...
try {
  ...
  // 截图记录文件选择界面
  await page.screenshot({ path: "tmp/TC-004文件选择.png" });
...
console.log("\n=== TC-002: 响应式设计（桌面端） ===");
// 已经是桌面端尺寸，截图记录
await page.screenshot({ path: "tmp/TC-002响应式设计_桌面端.png" });

// 测试平板端响应式设计
console.log("\n=== TC-002: 响应式设计（平板端） ===");
await page.setViewportSize({ width: 768, height: 1024 });
await page.waitForTimeout(1000);
await page.screenshot({ path: "tmp/TC-002响应式设计_平板端.png" });

// 测试移动端响应式设计
console.log("\n=== TC-002: 响应式设计（移动端） ===");
await page.setViewportSize({ width: 375, height: 667 });
await page.waitForTimeout(1000);
await page.screenshot({ path: "tmp/TC-002响应式设计_移动端.png" });
...
try {
  ...
  if (fileInput) {
    // 尝试上传不支持的格式（.txt文件）
    await fileInput.setInputFiles("/Users/cobbli/Desktop/storyweaver/test-files/test.txt");
    ...
    // 截图记录错误提示
    await page.screenshot({ path: "tmp/TC-005文件格式验证.png" });
...
try {
  ...
  // 截图记录
  await page.screenshot({ path: "tmp/TC-006文件大小验证.png" });
